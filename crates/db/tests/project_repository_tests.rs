...
async fn test_create_and_get_project() {
    ...
    assert!(create_result.is_ok(), "Failed to create project: {:?}", create_result);
    
    // Test get by id
    let get_result = repo.get_by_id(project_id).await;
    assert!(get_result.is_ok(), "Failed to get project: {:?}", get_result);
    
    let retrieved_project = get_result.unwrap();
    assert!(retrieved_project.is_some(), "Project not found after creation");
...
async fn test_get_nonexistent_project() {
    ...
    assert!(result.is_ok(), "Failed to query for nonexistent project: {:?}", result);
...
async fn test_project_with_complex_data() {
    ...
    // Create and retrieve
    let create_result = repo.create(project.clone()).await;
    assert!(create_result.is_ok(), "Failed to create complex project: {:?}", create_result);
...
async fn test_project_serialization_deserialization() {
    ...
    assert_eq!(retrieved.name(), "Project with 'quotes' and \"double quotes\"");
